...
`Robert S. Pyndick and Daniel L. Rubinfeld (Microeconomics, 2018 p.15)`

## News

![](potret.png){.absolute top=200 left=0 width="350" height="300"}
![](potret2.png){.absolute top=50 right=50 width="450" height="250"}
...
```
## For men
$$\hat {lwage} = \hat \beta_0 - .164 \,belavg + .016 \,abvavg + other \,factor$$
For men, those with below average looks are estimated to earn about 16.4% less than an average-
looking man who is the same in other respects (including education, experience, tenure, marital status, and race). The effect is statistically different from zero. Men with above average 
looks are estimated to earn only 1.6% more than men with average looks, and the effect is not statistically significant.

## For women

$$\hat {lwage} = \hat \beta_0 - .124 \,belavg + .035 \,abvavg + other \,factor$$
A woman with below average looks earns about 12.4% less than an otherwise comparable 
average-looking woman.As was the case for men, the estimate on abvavg is much smaller in magnitude and not statistically different from zero.
